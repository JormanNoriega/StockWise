...
const ConsultarVentas = () => {
    ...
    setFilteredVentas(
      ventas.filter((venta) => {
        ...
        const isWithinDateRange =
          (!fechaInicio || fechaVenta >= startOfDay(fechaInicio)) &&
          (!fechaFin || fechaVenta <= endOfDay(fechaFin));
        const matchesEmpleadoFilter =
          filterValue === "" ||
  ...
  const generateVentasPDF = () => {
    ...
    doc.autoTable({
      ...
      head: [
        ["ID Venta", "Empleado", "Total de la Venta", "Fecha de la Venta"],
      ],
  ...
  const generateDetallePDF = () => {
    ...
    doc.text(
      `Total: ${selectedVenta.totalVenta}`,
      14,
        ...
        <div className="table-card">
          ...
          <div className="search-bar">
            <input
              ...
              autoComplete="off"
              onChange={(e) =>
                setFechaInicio(e.target.value ? new Date(e.target.value) : null)
              }
            />
            <input
              type="text"
            ...
            <input
              ...
              autoComplete="off"
              onChange={(e) =>
                setFechaFin(e.target.value ? new Date(e.target.value) : null)
              }
            />
          </div>
          ...
          <div className="print-button-container">
            <button onClick={generateVentasPDF}>
              <FaFilePdf
                style={{
                  marginLeft: "10px",
                  marginRight: "10px",
          ...
          {selectedVenta && (
            <div
              className={`overlay ${isExiting ? "hidden" : "visible"}`}
              onClick={handleCloseModal}
            >
              <div
                className={`detalle-venta-card ${isExiting ? "exiting" : ""}`}
                onClick={(e) => e.stopPropagation()}
              >
                <h2>Detalle de la Venta {selectedVenta.idVenta}</h2>
                <p>
                  <strong>Total de la Venta: </strong>{" "}
                  {selectedVenta.totalVenta}
                </p>
                <p>
                  <strong>Fecha de la Venta: </strong>{" "}
                  {formatFecha(selectedVenta.fechaVenta)}
                </p>
                <h3>Productos:</h3>
                <ul>
                  {selectedVenta.detallesVenta &&
                  selectedVenta.detallesVenta.length > 0 ? (
                    selectedVenta.detallesVenta.map((detalle) => (
                      <li key={detalle.idDetalleVenta}>
                        ...
                        <p>
                          <strong>Sub Total: </strong> {detalle.subTotal}
                        </p>
                      </li>
                    ))
                  ) : (
                ...
                <div className="report-button-card">
                  <button
                    ...
                    onClick={generateDetallePDF}
                  >
                    Generar PDF de la factura
                  </button>
                </div>
                <div className="cerrar-button-card">
                  <button className="cerrar-button" onClick={handleCloseModal}>
                    Cerrar
                  </button>
                </div>
              </div>
